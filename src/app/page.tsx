...
export default function App() {
  ...
  function addToCart(p: Produto) {
    ...
    setCart((prev) => {
      ...
      if (ex) {
        if (ex.cartQtd >= p.qtd) {
          showToast("Estoque máximo: " + p.qtd);
          return prev;
        }
  ...
  async function finalizarVenda() {
    const total = cart.reduce(
      (s, i) =>
        s + parseFloat(i.preco) * i.cartQtd,
      0
    );
   ...
   * ==========================================================
   *
   * Agora o fechamento é separado por:
   *
   * Forma de pagamento
   * ├── Valor total
   * └── Produtos
   *     ├── Produto A - quantidade - valor
   *     ├── Produto B - quantidade - valor
   *     └── Produto C - quantidade - valor
   *
   * Também mantemos o resumo geral de produtos.
   */
  ...
  const resumoFechamento = () => {
    ...
    vendas.forEach((v) => {
      ...
      /*
       * Soma o valor total daquela forma de pagamento.
       */
      porPagamento[pagamentoAtual].total +=
        Number(v.total);

      itens.forEach((it) => {
        const nome = it.nome;
        const qtd = Number(it.cartQtd);
                      ...
                      {Object.entries(
                        ...
                        ([
                          ...
                          >
                            ...
                            {/* PRODUTOS DA FORMA DE PAGAMENTO */}
