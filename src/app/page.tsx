...
export default function App() {
  ...
  const [apagandoFechamento, setApagandoFechamento] = useState<number | null>(null);
  ...
  async function fetchHistoricoVendas() {
    try {
      const r = await fetch("/api/vendas?historico=true", { cache: "no-store" });
      const data = await r.json();
      if (Array.isArray(data)) setVendasHistorico(data);
      else setVendasHistorico(vendas);
    } catch {
  ...
  async function fetchFechamentos() {
    try {
      const r = await fetch("/api/fechamentos", { cache: "no-store" });
      if (!r.ok) return;
      const data = await r.json();
  ...
  useEffect(() => {
    if (!logged) return;
    if (tab === "log") fetchHistoricoVendas();
    if (tab === "fechamentos") fetchFechamentos();
  }, [tab, logged]);

  function doLogin() {
    if (loginForm.user === "freezer" && loginForm.pass === savedPass) {
      setLogged(true);
  ...
  function addToCart(produto: Produto) {
    if (produto.qtd <= 0) return showToast("Sem estoque");
    ...
    setCart((prev) => {
      ...
      if (existing) {
        ...
        return prev.map((i) =>
          i.id === produto.id ? { ...i, cartQtd: i.cartQtd + 1 } : i
        );
      }
      return [...prev, { ...produto, cartQtd: 1 }];
    });
  }
  ...
  async function finalizarVenda() {
    ...
    const total = cart.reduce(
      (s, i) => s + (parseFloat(i.preco) || 0) * i.cartQtd,
      0
    );
    ...
    try {
      const r = await fetch("/api/vendas", {
        ...
        body: JSON.stringify({ itens: cart, total, pagamento, descricao }),
      });
      const data = await r.json();
      if (!r.ok) throw new Error(data.error || "Erro ao salvar venda");
      ...
      showToast(e?.message || "Erro ao salvar venda");
    }
  }

  function criarResumo(lista: Venda[]): Resumo {
    const mapa: Record<string, ProdutoResumo> = {};
    const porPagamento: Record<string, PagamentoResumo> = {};
    const porDescricao: Record<string, DescricaoResumo> = {};
    ...
    lista.forEach((venda) => {
      ...
      const pg = String(venda.pagamento || "outro").trim().toLowerCase();

      if (!porPagamento[pg])
        porPagamento[pg] = { total: 0, produtos: {} };
      porPagamento[pg].total += Number(venda.total || 0);

      const original = String(venda.descricao || "").trim();
      const chave = normalizarDescricao(original);
      let grupo: DescricaoResumo;

      if (chave) {
        if (!porDescricao[chave])
          porDescricao[chave] = { nome: original, total: 0, produtos: {} };
        grupo = porDescricao[chave];
        grupo.total += Number(venda.total || 0);
      } else {
        grupo = semDescricao;
        semDescricao.total += Number(venda.total || 0);
      }

      itens.forEach((item) => {
        const nome = String(item.nome || "").trim();
        const qtd = Number(item.cartQtd) || 0;
        const valor = (parseFloat(item.preco) || 0) * qtd;
    ...
    return {
      mapa,
      porPagamento,
      porDescricao: Object.entries(porDescricao).sort((a, b) =>
        a[1].nome.localeCompare(b[1].nome, "pt-BR", { sensitivity: "base" })
      ),
      semDescricao,
      totalGeral: lista.reduce((s, v) => s + Number(v.total || 0), 0),
    };
  }
  ...
  async function fazerFechamento() {
    if (!vendas.length) {
      ...
      return showToast("Não existem vendas para fechar.");
    }
    ...
    try {
      ...
      const data = await r.json();
      if (!r.ok) throw new Error(data.error || "Erro ao fazer fechamento");
      ...
      setShowConfirmFechamento(false);
      setTab("fechamentos");
      showToast(data.message || "Fechamento realizado!");
    } catch (e: any) {
      showToast(e?.message || "Erro ao fazer fechamento");
    } finally {
  ...
  async function deletarFechamento(id: number) {
    if (!confirm(`Apagar o relatório de fechamento #${id}? Esta ação não apaga as vendas do Log.`))
      return;

    setApagandoFechamento(id);
    try {
      const r = await fetch(`/api/fechamentos?id=${id}`, { method: "DELETE" });
      const data = await r.json().catch(() => ({}));
      if (!r.ok) throw new Error(data.error || "Erro ao apagar relatório");
  ...
  async function deletarProduto(id: number) {
    ...
    try {
      const r = await fetch("/api/produtos?id=" + id, { method: "DELETE" });
  ...
  async function deletarVenda(id: number) {
    if (!confirm("Apagar venda #" + id + "? O estoque vai voltar.")) return;
    try {
      const r = await fetch("/api/vendas?id=" + id, { method: "DELETE" });
      const data = await r.json().catch(() => ({}));
      if (!r.ok) throw new Error(data.error || "Erro ao apagar venda");
      await fetchVendas();
      await fetchHistoricoVendas();
      await fetchProdutos();
      showToast("Venda apagada");
    } catch (e: any) {
      showToast(e?.message || "Erro ao apagar venda");
    }
  }

  const resumoAtual = useMemo(() => criarResumo(vendas), [vendas]);

  const totalCarrinho = cart.reduce(
    (s, i) => s + (parseFloat(i.preco) || 0) * i.cartQtd,
    0
  );
  const totalQtdCarrinho = cart.reduce((s, i) => s + i.cartQtd, 0);
  const produtosAtivos = produtos.filter((p) => p.qtd > 0);

  const vendasLogFiltradas = useMemo(() => {
    const busca = logBusca.trim().toLocaleLowerCase("pt-BR");
    if (!busca) return vendasHistorico;
  ...
  if (!logged) {
    return (
      <div className={`min-h-screen flex items-center justify-center p-6 ${bg}`}>
        ...
        <div className={`w-full max-w-[380px] p-8 rounded-[24px] border ${card}`}>
          ...
          />
          <button onClick={doLogin} className="w-full mt-5 py-3 rounded-xl bg-[#D6FF57] text-black font-bold">
            Entrar
          </button>
        </div>
      </div>
  ...
  return (
    <div className={`min-h-screen flex flex-col items-center overflow-x-hidden ${bg}`}>
      <link
        rel="stylesheet"
        href="https://fonts.googleapis.com/css2?family=Material+Symbols+Rounded:opsz,wght,FILL,GRAD@20..48,100..700,0..1,-50..200"
      />
      ...
      <header className="w-full max-w-[1200px] px-5 sm:px-6 py-4 flex justify-between">
        ...
        <span className="text-xs opacity-50">
          {vendas.length ? `${vendas.length} venda${vendas.length > 1 ? "s" : ""} em aberto` : "Nenhuma venda em aberto"}
        </span>
      </header>
      ...
      <main className="w-full max-w-[1200px] flex-1 px-4 sm:px-6 pb-[170px] pt-2">
        {tab === "home" && (
          <section>
            ...
            <p className="text-xs opacity-60">{produtosAtivos.length} produtos disponíveis</p>
              ...
              {produtosAtivos.map((p) => (
                <div key={p.id} className={`rounded-2xl border overflow-hidden ${card}`}>
                  ...
                  <div className="p-3">
                    ...
                    <p className="text-[11px] opacity-50">Estoque: {p.qtd}</p>
                    <div className="flex justify-between items-center mt-2 gap-2">
                      <b className="text-sm">R$ {p.preco}</b>
                      <button onClick={() => addToCart(p)} className="w-8 h-8 rounded-full bg-[#D6FF57] text-black flex items-center justify-center">
        ...
        {tab === "pdv" && (
          <section className="max-w-[700px] mx-auto">
            ...
            <AddProduto fetchProdutos={fetchProdutos} showToast={showToast} />

            <h2 className="text-xl font-bold mt-6 mb-3">Produtos no banco</h2>
            <div className={`rounded-2xl border divide-y ${card} divide-zinc-800 overflow-hidden`}>
              {produtos.map((p) => (
                <div key={p.id} className={`p-4 flex gap-3 items-center ${p.qtd <= 0 ? "opacity-40" : ""}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold truncate">{p.nome} {p.qtd <= 0 && "(ESGOTADO)"}</p>
                    <p className="text-xs opacity-60">R$ {p.preco} • {p.qtd} un</p>
                  </div>
                  <button
                    onClick={async () => {
                      const quantidade = prompt("Nova qtd:", String(p.qtd));
                      if (quantidade === null) return;
                      await fetch("/api/produtos", {
                        method: "PUT",
                        headers: { "Content-Type": "application/json" },
                        body: JSON.stringify({ id: p.id, qtd: Number(quantidade) }),
                      });
                      fetchProdutos();
                    }}
                  ...
                  <button onClick={() => deletarProduto(p.id)} className="w-8 h-8 rounded-full bg-red-500/20 text-red-400">
                    <span className="material-symbols-rounded text-[18px]">delete</span>
                  </button>
                </div>
              ))}
              {!produtos.length && (
                <p className="p-6 text-center text-xs opacity-50">Nenhum produto cadastrado.</p>
              )}
            </div>
          </section>
        )}
          ...
          <section className="max-w-[800px] mx-auto space-y-4">
            ...
            <div className={`p-5 rounded-2xl border ${card}`}>
              ...
              <div className="mt-4 space-y-3">
                ...
                {Object.keys(resumoAtual.semDescricao.produtos).length > 0 && (
                  <ResumoDescricao dados={resumoAtual.semDescricao} />
                )}
          ...
          <section className="max-w-[800px] mx-auto">
            <div className="flex justify-between items-end gap-3">
              <div>
                ...
                <p className="text-xs opacity-60">Histórico dos ciclos encerrados</p>
              </div>
              <button
                onClick={fetchFechamentos}
                className="px-4 py-2 rounded-xl border text-xs"
              >
                <span className="material-symbols-rounded align-middle text-[18px]">refresh</span>
                Atualizar
              </button>
            </div>
            ...
            <div className="mt-5 space-y-3">
              {fechamentos.map((f, index) => {
                const aberto = fechamentoAberto === f.id;
                const resumoSalvo = normalizarResumo(f.dados ?? f.resumo);
                const total = Number(f.total ?? resumoSalvo?.totalGeral ?? 0);

                return (
                  <div key={f.id} className={`rounded-2xl border overflow-hidden ${card}`}>
                    <button
                      onClick={() => setFechamentoAberto(aberto ? null : f.id)}
                      className="w-full p-4 flex items-center justify-between text-left"
                    >
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-xl bg-[#D6FF57] text-black flex items-center justify-center font-bold">
                          #{numeroFechamento(f, index)}
                        </div>
                        <div>
                          <p className="font-bold">#{numeroFechamento(f, index)} {formatarDataFechamento(f.created_at)}</p>
                          <p className="text-xs opacity-50">{formatarData(f.created_at)}</p>
                        </div>
                      </div>
                      <div className="flex items-center gap-2">
                        <b>{dinheiro(total)}</b>
                        <span className="material-symbols-rounded">{aberto ? "expand_less" : "expand_more"}</span>
                      </div>
                    </button>

                    {aberto && (
                      <div className="border-t border-zinc-800 p-4">
                        <RenderResumoSalvo resumo={resumoSalvo} card={card} />
                        <button
                          disabled={apagandoFechamento === f.id}
                          onClick={() => deletarFechamento(f.id)}
                        ...
                        >
                          <span className="material-symbols-rounded align-middle mr-1">delete</span>
                          {apagandoFechamento === f.id ? "Apagando..." : "Apagar relatório"}
                        </button>
                      </div>
                    )}
                  </div>
              ...
              {!fechamentos.length && (
                <div className={`p-8 rounded-2xl border text-center ${card}`}>
                  <span className="material-symbols-rounded text-4xl opacity-30">receipt_long</span>
                  <p className="mt-2 font-semibold">Nenhum fechamento</p>
                  <p className="text-xs opacity-50 mt-1">Faça o primeiro fechamento pelo Dashboard.</p>
                </div>
              )}
            </div>
          </section>
        ...
        {tab === "log" && (
          <section className="max-w-[800px] mx-auto">
            ...
            <p className="text-xs opacity-60">Todas as vendas feitas</p>
            <div className={`mt-4 p-3 rounded-2xl border ${card}`}>
              <div className="flex items-center gap-2">
                <span className="material-symbols-rounded opacity-50">search</span>
                <input
                  value={logBusca}
                ...
                return (
                  <div key={v.id} className={`p-4 rounded-2xl border ${card}`}>
                    <div className="flex justify-between gap-3">
                      <div>
                        <b>Venda #{v.id}</b>
                        <p className="text-[11px] opacity-50 mt-1">{formatarData(v.created_at)}</p>
                      </div>
                      <b>{dinheiro(Number(v.total))}</b>
                    </div>
                    <div className="mt-3 space-y-1">
                      {itens.map((item, i) => (
                        <div key={i} className="flex justify-between text-xs">
                          <span>{item.cartQtd}x {item.nome}</span>
                          <span className="opacity-60">{dinheiro((parseFloat(item.preco) || 0) * item.cartQtd)}</span>
                        </div>
                      ))}
                    </div>
                    <div className="mt-3 flex flex-wrap gap-2">
                      <span className="text-[10px] px-2 py-1 rounded-full bg-[#D6FF57] text-black uppercase">{v.pagamento}</span>
                      {v.fechamento_id && <span className="text-[10px] px-2 py-1 rounded-full bg-zinc-800">Fechamento #{v.fechamento_id}</span>}
                    </div>
                    {v.descricao && <div className="mt-2 p-2 rounded-xl bg-zinc-800 text-xs">📝 {v.descricao}</div>}
                    <button onClick={() => deletarVenda(v.id)} className="mt-3 w-full py-2 rounded-xl bg-red-500/10 text-red-400 text-xs">
                      Apagar venda
                    </button>
                  </div>
                );
              })}
              {!vendasLogFiltradas.length && <p className="text-center text-xs opacity-50 py-8">Nenhuma venda encontrada.</p>}
            </div>
          </section>
        )}
          ...
          <section className="max-w-[520px] mx-auto">
            ...
            <div className={`mt-6 p-6 rounded-2xl border space-y-6 ${card}`}>
              <div className="flex justify-between items-center">
                ...
                <button onClick={() => setDark(!dark)} className="px-4 py-2 rounded-full border">
                  {dark ? "Dark" : "Light"}
                </button>
              </div>
              <button
      ...
      {cart.length > 0 && (
        <div className={`fixed bottom-[108px] left-1/2 -translate-x-1/2 w-[calc(100%-24px)] max-w-[460px] rounded-[24px] p-4 shadow-2xl z-40 border ${card}`}>
          <div className="flex justify-between items-center mb-3">
            <b><span className="material-symbols-rounded align-middle mr-1">shopping_cart</span>{totalQtdCarrinho} itens</b>
            <b>{dinheiro(totalCarrinho)}</b>
          </div>
          <div className="max-h-[140px] overflow-auto space-y-2 mb-3">
            {cart.map((item) => (
              <div key={item.id} className="flex items-center gap-2 bg-zinc-800 rounded-xl p-2 text-white">
                <img src={item.imagem} className="w-8 h-8 bg-white rounded object-contain" />
                <span className="flex-1 text-xs truncate">{item.nome}</span>
                <button onClick={() => setCart(c => c.map(i => i.id === item.id ? {...i, cartQtd: Math.max(1, i.cartQtd - 1)} : i))} className="w-7 h-7 rounded-full bg-zinc-700">-</button>
                <span className="w-5 text-center text-sm">{item.cartQtd}</span>
                <button onClick={() => addToCart(produtos.find(p => p.id === item.id) || item)} className="w-7 h-7 rounded-full bg-zinc-700">+</button>
                <button onClick={() => setCart(c => c.filter(i => i.id !== item.id))} className="w-7 h-7 rounded-full bg-red-500/20 text-red-400">×</button>
              </div>
            ))}
          </div>
          <button onClick={() => setShowCheckout(true)} className="w-full py-3 rounded-xl bg-[#D6FF57] text-black font-bold">
            Finalizar Venda
          </button>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/70 z-[100] flex items-end md:items-center justify-center p-4">
          <div className={`w-full max-w-[460px] rounded-[24px] p-6 space-y-4 border ${card}`}>
            <div className="flex justify-between">
              <h2 className="font-bold text-xl">Finalizar</h2>
              <button onClick={() => setShowCheckout(false)}><span className="material-symbols-rounded">close</span></button>
            </div>
            <p className="text-sm opacity-60">{totalQtdCarrinho} itens • {dinheiro(totalCarrinho)}</p>
            <div className="grid grid-cols-4 gap-2">
              {["pix", "dinheiro", "cartao", "vale"].map((m) => (
                <button key={m} onClick={() => setPagamento(m)} className={`p-3 rounded-xl border text-xs uppercase ${pagamento === m ? "bg-[#D6FF57] text-black border-[#D6FF57]" : "border-zinc-700"}`}>
                  {m}
                </button>
              ))}
            </div>
            ...
            />
            <button onClick={finalizarVenda} className="w-full py-4 rounded-xl bg-[#D6FF57] text-black font-bold">
              Confirmar {dinheiro(totalCarrinho)}
            </button>
          </div>
        </div>
      )}
      ...
      {showConfirmFechamento && (
        <div className="fixed inset-0 bg-black/75 z-[150] flex items-center justify-center p-4">
          <div className={`w-full max-w-[430px] rounded-[24px] p-6 border ${card}`}>
            ...
            <p className="text-sm opacity-60 text-center mt-2">O ciclo atual será encerrado.</p>
            <div className="mt-4 p-4 rounded-2xl bg-zinc-800 text-white">
              <div className="flex justify-between text-sm"><span>Vendas</span><b>{vendas.length}</b></div>
              <div className="flex justify-between mt-2"><span>Total</span><b className="text-[#D6FF57]">{dinheiro(resumoAtual.totalGeral)}</b></div>
            </div>
            <p className="text-xs opacity-50 text-center mt-4">As vendas não serão apagadas. Elas ficarão no Log e no fechamento histórico.</p>
            <div className="grid grid-cols-2 gap-3 mt-5">
              <button disabled={fazendoFechamento} onClick={() => setShowConfirmFechamento(false)} className="py-3 rounded-xl border border-zinc-700">Cancelar</button>
              <button disabled={fazendoFechamento} onClick={fazerFechamento} className="py-3 rounded-xl bg-[#D6FF57] text-black font-bold">{fazendoFechamento ? "Fechando..." : "Confirmar"}</button>
            </div>
          </div>
        </div>
      )}
      ...
      <nav className="fixed bottom-4 left-1/2 -translate-x-1/2 w-[calc(100%-20px)] max-w-[520px] z-50">
        <div className="grid grid-cols-5 items-center rounded-[28px] p-2 bg-[#151517] border border-zinc-800 shadow-2xl overflow-hidden">
          {NAV_TABS.map((nav) => {
            const ativo = tab === nav.id;
            return (
              <button key={nav.id} onClick={() => setTab(nav.id)} className={`relative h-[56px] w-full flex items-center justify-center rounded-[22px] transition-all ${ativo ? "bg-[#D6FF57] text-black" : "text-white opacity-50"}`}>
                <span className="material-symbols-rounded text-[26px]">{nav.icon}</span>
              </button>
            );
          })}
        </div>
...
function Toast({ text }: { text: string }) {
  return <div className="fixed top-6 left-1/2 -translate-x-1/2 bg-zinc-900 text-white px-5 py-3 rounded-full text-sm z-[300] border border-zinc-700 max-w-[90%] text-center">{text}</div>;
}

function ResumoPagamento({ nome, dados }: { nome: string; dados: PagamentoResumo }) {
  return (
    <div className="rounded-2xl bg-zinc-800 text-white overflow-hidden">
      <div className="p-3 flex justify-between border-b border-zinc-700">
        <b className="uppercase text-sm">{nome}</b>
        <b className="text-[#D6FF57]">{dinheiro(dados.total)}</b>
      </div>
      <div className="p-3 space-y-2">
        {Object.entries(dados.produtos).map(([n, p]) => (
          <div key={n} className="flex justify-between text-xs">
            <span><b className="text-[#D6FF57]">{p.qtd}x</b> {n}</span>
            <span className="opacity-70">{dinheiro(p.total)}</span>
          </div>
        ))}
      </div>
    </div>
...
function ResumoDescricao({ dados }: { dados: DescricaoResumo }) {
  return (
    <div className="rounded-2xl bg-zinc-800 text-white overflow-hidden">
      <div className="p-3 flex justify-between border-b border-zinc-700">
        <b>{dados.nome}</b>
...
function RenderResumoSalvo({ resumo, card }: { resumo: Resumo | null; card: string }) {
  if (!resumo) {
    return (
      <div className={`p-4 rounded-xl border ${card}`}>
        <p className="text-xs opacity-50">
          Este relatório não possui resumo detalhado salvo.
          Verifique se a API /api/fechamentos está salvando o campo dados ou resumo.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      <div className={`p-4 rounded-2xl border ${card}`}>
        <p className="text-xs opacity-50">Total do fechamento</p>
        <p className="text-3xl font-bold">{dinheiro(resumo.totalGeral)}</p>
      </div>
      ...
      {Object.keys(resumo.porPagamento).length > 0 && (
        <div>
          ...
          <div className="space-y-2">
            {Object.entries(resumo.porPagamento).map(([nome, dados]) => (
              <ResumoPagamento key={nome} nome={nome} dados={dados} />
            ))}
          </div>
        </div>
      )}
      ...
      {resumo.porDescricao.length > 0 && (
        <div>
          <p className="font-bold mb-2">👤 Por descrição</p>
          <div className="space-y-2">
            {resumo.porDescricao.map(([chave, dados]) => (
              <ResumoDescricao key={chave} dados={dados} />
            ))}
          </div>
        </div>
      ...
      {Object.keys(resumo.semDescricao.produtos).length > 0 && (
        <div>
          <p className="font-bold mb-2">📝 Sem descrição</p>
          <ResumoDescricao dados={resumo.semDescricao} />
        </div>
      )}

      <div>
        <p className="font-bold mb-2">📦 Produtos vendidos</p>
        <div className={`p-4 rounded-2xl border ${card} space-y-2`}>
          {Object.entries(resumo.mapa).map(([nome, dados]) => (
            <div key={nome} className="flex justify-between text-sm">
              <span>{dados.qtd}x {nome}</span>
              <b>{dinheiro(dados.total)}</b>
            </div>
          ))}
        </div>
      </div>
...
function AddProduto({
  ...
  async function save() {
    ...
    try {
      ...
      const data = await r.json();
      if (!r.ok) throw new Error(data.error || "Erro banco");
      setForm({ nome: "", preco: "", qtd: "", imagem: "" });
      await fetchProdutos();
      showToast(data.qtd ? `Estoque somado! Agora ${data.qtd} un` : "Produto salvo!");
    } catch (e: any) {
  ...
  return (
    <div className="mt-6 p-6 rounded-2xl bg-zinc-900 border border-zinc-800 space-y-4">
      <h3 className="font-bold">➕ Adicionar novo produto</h3>
      
      <div className="grid grid-cols-4 gap-2">
        {GALERIA.map(([nome, url]) => (
          <button
            key={nome}
            onClick={() => setForm({ ...form, imagem: url, nome: form.nome || nome })}
            className={`h-20 rounded-xl bg-white p-1 border-2 ${form.imagem === url ? "border-[#D6FF57]" : "border-transparent"}`}
          >
      ...
      {form.imagem && (
        <div className="flex gap-3 items-center p-2 bg-zinc-800 rounded-xl text-white">
          <img src={form.imagem} className="w-12 h-12 bg-white rounded p-1 object-contain" />
          <span className="text-sm">{form.nome}</span>
        </div>
      )}
      ...
      <button onClick={save} disabled={loading} className="w-full py-3 rounded-xl bg-[#D6FF57] text-black font-bold disabled:opacity-50">
