...
export async function POST(req: Request) {
  try {
    ...
    if (!nome || preco === undefined || qtd === undefined || !imagem) {
      return NextResponse.json(
        { error: "Nome, preço, quantidade e imagem são obrigatórios" },
        { status: 400 }
      );
    }
    ...
    if (exist.rows.length > 0) {
      ...
      await db.execute({
        sql: "UPDATE produtos SET qtd = ?, preco = ?, imagem = ? WHERE id = ?",
        args: [novaQtd, preco, imagem, p.id],
      });
    ...
    await db.execute({
      sql: "INSERT INTO produtos (nome, preco, qtd, imagem) VALUES (?, ?, ?, ?)",
      args: [nome, preco, qtd, imagem],
    });
